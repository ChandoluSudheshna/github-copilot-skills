...
activities = {
    ...
    "Debate Team": {
        "description": "Prepare for debate competitions and sharpen persuasive speaking",
...
def unregister_from_activity(activity_name: str, email: str):
    ...
    # Remove all matching occurrences of the email
    original = list(activity["participants"])
    updated = [p for p in original if p != email]

    if len(updated) == len(original):
        raise HTTPException(status_code=404, detail="Participant not found")

    activity["participants"] = updated
