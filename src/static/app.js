  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants HTML (compact, with delete buttons)
        const participantsHtml = details.participants && details.participants.length
          ? `<div class="participants-section"><strong>Participants</strong><ul class="participants-list">${details.participants.map(p => `<li class="participant-item"><span class="participant-email">${p}</span><button class="delete-participant" data-email="${p}" aria-label="Remove ${p}">✖</button></li>`).join("")}</ul></div>`
        ...
        // Attach activity name to the card for handlers
          ...
          btn.addEventListener('click', async (e) => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                if (availabilityEl) {
                  availabilityEl.innerHTML = `<strong>Availability:</strong> ${details.max_participants - participantsCount} spots left`;
                }

                // if no participants remain, show placeholder
                const list = activityCard.querySelector('.participants-list');
                if (!list || participantsCount === 0) {
